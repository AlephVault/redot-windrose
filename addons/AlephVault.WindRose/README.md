...
- `signal orientation_changed(AlephVault__WindRose.Utils.DirectionUtils.Direction)`: A signal triggered
...
- `speed: float`: Sets or returns the speed of this entity. The speed is expressed in pixels. The signal
  `speed_changed` is triggered with the new speed on assignment. It is always clamped to be greater than
