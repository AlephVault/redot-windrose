...
const _WATER_WHEEL_TEXTURE := preload("res://addons/AlephVault.WindRose.LPC/images/farm/lpc-farm-water-wheel.png")
...
## The wheel spin direction.
@export var spin: Spin = Spin.CLOCKWISE:
	set(value):
		if spin == value:
			return
		spin = value
...
func _setup_sprite() -> void:
	texture = _WATER_WHEEL_TEXTURE
	region_enabled = true
	region_filter_clip_enabled = true
	region_rect = Rect2(Vector2.ZERO, Vector2(64, 384))
	hframes = 1
...
func _reset_animation_frame() -> void:
	_accumulated = 0
	_frame = _FRAME_COUNT - 1 if spin == Spin.COUNTER_CLOCKWISE else 0
...
func _update(delta: float) -> void:
	if fps > 0:
		...
		while _accumulated >= frame_time:
			var step := -1 if spin == Spin.COUNTER_CLOCKWISE else 1
